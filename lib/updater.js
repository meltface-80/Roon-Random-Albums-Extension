...
function applyStaged(stagedDir, targetDir, opts) {
  ...
  copyOverlay(stagedDir, targetDir, [".git", "node_modules", ".update", "config.json"]);
...
function createUpdater(o) {
  ...
  function getStatus() {
    return {
      ...
      available: state.available, html_url: state.html_url, notes: state.notes,
  ...
  async function checkNow() {
    ...
    try {
      ...
      if (rel) {
        ...
        state.notes = (rel.body || "").trim().slice(0, 2000) || null;
        state.source = src;
        state._downloadUrl = tarAsset ? tarAsset.browser_download_url
                            : (rel.tarball_url || rel._tarball || null);
        state.available = !!(parseVer(ver) && parseVer(currentVersion) && verGt(ver, currentVersion));
      } else {
  ...
  async function apply() {
    ...
    try {
      ...
      // shell:true ensures tar is found via the shell's PATH (needed in Docker/Alpine
      // where spawnSync's inherited PATH may not include /bin or /usr/bin).
      const ex = spawnSync("tar", ["-xzf", dlFile, "-C", exRoot],
