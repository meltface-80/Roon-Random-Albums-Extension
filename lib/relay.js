...
// Stop the raspotify system service if it is currently active.
// raspotify bundles its own librespot which competes for the Avahi/mDNS slot,
// causing "Local name collision" and an immediate librespot crash (exit code 1).
function stopRaspotify() {
  try {
    const r = spawnSync("systemctl", ["is-active", "raspotify"],
      { timeout: 3000, encoding: "utf8" });
    if (r.stdout && r.stdout.trim() === "active") {
      console.log("[relay] stopping raspotify service (Avahi collision prevention)");
      console.log("[relay] Tip: permanently disable it with: systemctl disable raspotify");
      execFileSync("systemctl", ["stop", "raspotify"], { timeout: 8000, stdio: "ignore" });
      // Give Avahi a moment to release the service registration
      const wait = spawnSync("sleep", ["1"], { timeout: 2000 });
    }
  } catch (e) {
    // Not a systemd system or raspotify not installed — no action needed
  }
}
