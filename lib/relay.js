...
class ZoneRelay extends EventEmitter {
  constructor(zoneId, zoneName, appPort, debug) {
    ...
    this.clientsEver     = 0;
    this.audioKeyErrors  = 0;  // incremented each time Spotify refuses an audio key
  }
  ...
  toJSON() {
    return {
      ...
      audio_key_errors: this.audioKeyErrors
    };
  }

  // ── Private ───────────────────────────────────────────────────────────────

  _onLibrespotStderr(line) {
    if (line) console.log(`[librespot:${this.zoneName}] ${line}`);
    if (/audio key error|Unable to load key.*Service unavailable/i.test(line)) {
      this.audioKeyErrors++;
      if (this.audioKeyErrors === 1) {
        console.error(`\n[relay:${this.zoneName}] *** AUDIO KEY ERROR — Spotify refused to provide a decryption key ***`);
        console.error(`[relay:${this.zoneName}]   Most likely cause 1: your Spotify account is FREE, not Premium.`);
        console.error(`[relay:${this.zoneName}]     librespot only works with Spotify Premium accounts.`);
        console.error(`[relay:${this.zoneName}]   Most likely cause 2: this librespot binary (raspotify 0.8.0, Nov 2025)`);
        console.error(`[relay:${this.zoneName}]     may be outdated and blocked by Spotify's current API.`);
        console.error(`[relay:${this.zoneName}]     Fix: apt-get update && apt-get upgrade raspotify`);
        console.error(`[relay:${this.zoneName}]     Or build from source: https://github.com/librespot-org/librespot\n`);
      } else if (this.audioKeyErrors <= 5) {
        console.error(`[relay:${this.zoneName}] audio key error #${this.audioKeyErrors} (see first error for diagnosis)`);
      }
    }
  }
  ...
  _librespotArgs(extraArgs) {
    // Cache dir stores the OAuth/Zeroconf credential blob so the device stays
    // authenticated across restarts without requiring the Spotify app to re-pair.
    const cacheDir = path.join(os.tmpdir(), `librespot-cache-${this.zoneId.slice(0, 8)}`);
