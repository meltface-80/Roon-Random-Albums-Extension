...
// Kill any librespot processes that would conflict with ours via Avahi.
// This catches:
//   - raspotify service (stopped via systemctl if active)
//   - orphaned librespot child processes left over from previous runs
//     (Node.js does NOT kill children automatically on Ctrl+C)
function killConflictingLibrespot(ourPid) {
  ...
  // 2. Kill any other librespot processes (not ours)
  try {
    const r = spawnSync("pgrep", ["-f", "librespot"], { timeout: 3000, encoding: "utf8" });
    const pids = (r.stdout || "").trim().split("\n").filter(Boolean)
      .map(Number).filter(p => p && p !== ourPid && p !== process.pid);
    if (pids.length) {
      console.log(`[relay] killing orphaned librespot PIDs: ${pids.join(", ")}`);
      for (const pid of pids) { try { process.kill(pid, "SIGTERM"); } catch {} }
    }
  } catch {}

  // Give Avahi 2 seconds to deregister the freed service slot
  spawnSync("sleep", ["2"], { timeout: 3000 });
  console.log("[relay] librespot conflict cleanup done");
}
